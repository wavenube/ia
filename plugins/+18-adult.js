...
const handler = async (m, { command, conn }) => {
  ...
  const sendNsfwImage = async (url, command) => {
    ...
    const button = {
      ...
      description: `_${command}_`,
      sections: [{ title: "Opciones", rows: [{ title: "Siguiente", rowId: `${command}` }] }],
      listType: 1
    };
    conn.sendMessage(m.chat, { image: { url: imageUrl }, caption: `_${command}_`.trim(), buttons: button, footer: 'Presiona el botón para ver otra imagen.' }, { quoted: m });
  };
  ...
  switch (command) {
    ...
    case 'yaoi2':
      ...
      const resErrorYaoi2 = (await axios.get(`https://raw.githubusercontent.com/BrunoSobrino/TheMystic-Bot-MD/master/src/JSON/yaoi2.json`)).data;
      const imageYaoi2 = jsonYaoi2.link || resErrorYaoi2[Math.floor(resErrorYaoi2.length * Math.random())];
      conn.sendMessage(m.chat, { image: { url: imageYaoi2 }, caption: `_${command}_`.trim(), buttons: [{ buttonId: `${command}`, buttonText: { displayText: 'Siguiente' }, type: 1 }], footer: 'Presiona el botón para ver otra imagen.' }, { quoted: m });
      break;
    case 'yuri':
    ...
    case 'yuri2':
      ...
      conn.sendMessage(m.chat, { image: { url: imageYuri2 }, caption: `_${command}_`.trim(), buttons: [{ buttonId: `${command}`, buttonText: { displayText: 'Siguiente' }, type: 1 }], footer: 'Presiona el botón para ver otra imagen.' }, { quoted: m });
      break;
  }
};
